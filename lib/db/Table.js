...
var Table = function () {
    ...
    Table.prototype.query = function query(branch, clauses) {
        ...
        var optimallyOrderedClauses = (0, _sortBy2.default)(clauses, function (clause) {
            if (_this2._willClauseUseIndexedAttribute(clause)) {
                return 1;
            }

            if (_this2._willClauseReduceResultSetSize(clause)) {
                return 2;
            }
        ...
        var reducer = function reducer(rows, clause) {
            ...
            if (!rows) {
                if (type === _constants.FILTER && payload.hasOwnProperty(idAttribute)) {
    ...
    Table.prototype._willClauseUseIndexedAttribute = function _willClauseUseIndexedAttribute(clause) {
        return clause.type === _constants.FILTER && clause.payload.hasOwnProperty(this.idAttribute);
    };

    Table.prototype._willClauseReduceResultSetSize = function _willClauseReduceResultSetSize(_ref) {
        var type = _ref.type;

        return type === _constants.FILTER || type === _constants.EXCLUDE;
    };
